...
| `boolean` | unsigned integer (§4.4) | no own wire type; encoded as `0`/`1` via the corelib bool helper. unlike every other integer-backed leaf type the value carries **no** width bound — a decoder reads every non-`0` as `true` and normalizes it (CORELIB_PLAN §4.4, §4.1.2) |
| `enum` | signed integer (§4.5) | no own wire type; carries the member's value. The **declared width is a normative validity bound** — the smallest signed type holding every declared constant, `i8`…`i64`; a value outside its range is `INVALID` (§7.1). A value inside it that names no constant is valid |
| `bitfield` | unsigned integer (§4.4) | no own wire type; flags packed by generated code at their `pos` bits. The **declared width is a normative validity bound** — the smallest unsigned type holding the highest declared `pos`, `u8`…`u64`; a value outside its range is `INVALID` (§7.1). An undeclared bit inside it is valid |
...
**Not reaching the wire is not the same as not binding it.** Three things are
unencoded but binding: an array's `count` (§3), a `string`/`blob`'s `maxlen`, and an
integer scalar's declared **width** — the integer scalars share one unsigned/signed
wire type regardless of width (table above), so the width is not encoded, but it
...
**An `enum` is a signed integer of a declared width (normative).** Its bound is not
the set of constants the schema declares but the **width of the smallest signed
integer type that holds every one of them** — `i8`, `i16`, `i32` or `i64`. A wire
value inside that width's range is valid even when the schema names no constant for
it; a value outside it is malformed input and **MUST** be reported `INVALID` (§7.1),
exactly as an over-width `i8` is. An enum `{RED = 1, GREEN = 2, BLUE = 3}` is bounded
as an `i8`: −128 .. 127. One declaring `{1, 2, 300}` is bounded as an `i16`.

The width is **signed whatever the constants are**, because the wire type is (§4.5).
Deriving it from their sign instead — `u8` for an all-non-negative enum — would make
an enum's *wire type* a function of its constants, so adding a negative constant
later would change the encoding rather than only the range.

Two reasons the bound is a width and not the set:

* **The set is not uniformly expressible.** Every other bound in this format is a
  width or a length, and that is what the decode path carries: where an array's
  elements are consumed inside the corelib loop, generated code never sees them
  individually and the bound must be handed over as an interval. A width fits that
  channel; a set does not. (A schema-declared bound is enforced by generated code —
  CORELIB_PLAN §6.3.)
* **Compatibility is carried on one vector, and it is the field id.** Field ids give
...
* **Adding a constant is not a breaking schema change** as long as it fits the width
  ...
  gaining `400` moves the bound from `i8` to `i16`, and a receiver built on the older
  schema rejects the wider value. A schema that wants headroom can reserve it by
  declaring a constant at the width it intends to keep.
* **Storage follows the declared width, not the constants.** A receiver **MUST** hold
  the field in a type that covers the full declared width — `i8` for `{1, 2, 3}`, not
...
* **A value the schema does not name is stored as received.** It is not masked, not
  ...
  (CORELIB_PLAN §6.6 — nothing the message chooses is allocated on the receiver).
  A target language whose native `enum` cannot hold an undeclared value (Java,
  Kotlin, Dart, Rust, a strict Python `Enum`) **MUST** materialize the field as the
  declared integer type with named constants beside it, rather than as that language's
  closed enum.

**A field's default must still name a constant.** §2 initializes every field to its
schema `default`, or to *the type's zero value* when none is given, and a sparse
encoder omits the field at exactly that value — so absence reconstructs it on every
receiver. An `enum` field **MUST** therefore either declare a `default` naming one of
the enum's constants, or belong to an enum that declares a constant with the value
`0`. An enum `{RED = 1, GREEN = 2}` whose field declares no `default` is **not** a
valid schema — its fields would initialize to `0`, which the width admits but the
schema gives no meaning to, leaving every receiver with an uninterpretable value it
never received. This is a schema-validity rule (§6), not a decode check. A `bitfield`
needs no counterpart clause — its zero value is the "no flags set" combination and is
always meaningful.

**A `bitfield` is an unsigned integer of a declared width (normative).** Its bound is
the **width of the smallest unsigned integer type that holds its highest declared
`pos`** — `u8`, `u16`, `u32` or `u64`. A bitfield declaring positions 0, 1 and 3 is
bounded as a `u8`: `0 .. 255`. A value inside that range is valid whichever bits it
carries, including undeclared ones; a value outside it is malformed input and **MUST**
be reported `INVALID` (§7.1).

The two consequences of the enum rule hold unchanged here, with `pos` in place of
constants: adding a flag is **not** a breaking schema change while it fits the
declared width and becomes one when it widens the type, and a receiver **MUST** hold
the field in a type covering that full width. A field whose declared positions are
0..3 is bounded `0 .. 255` — the width its declaration implies, not the mask its
flags form.

**Undeclared bits are not masked away on decode**, for the same reason an unnamed
enum value is not: masking would turn malformed-looking input into a *declared*
combination and report it Ok. Whether those bits survive a decode-and-re-encode round
trip is a **storage** question the format deliberately leaves open — a target that
materializes the field as one integer preserves them, one that materializes it as a
set of named booleans does not, and neither is more conformant than the other. A
schema that needs them preserved declares them.
...
- **The ≠-default test is per field — and a `sequence`-typed field is no
  ...
  default (init rule above), so the omission is value-preserving by construction —
  and it is the same test §3 applies to a compact scalar array, so the two array
  forms agree. A sequence whose value **differs** from that default is **framed**
  ...
  **What an empty frame denotes (normative).** A decoder **MUST** accept a
  ...
  the sequence occupies, and the positions do not agree:
  ...
  therefore stays readable: every frame it emits that this rule would have omitted
  ...
  *Rationale:* the predicate is the **conjunction of the per-field tests the
  ...
  handled by the same per-field test as everywhere else. §5.1 already requires
  exactly this composite predicate in order to decide whether an interior
...
- **Sparse omission reaches into wrapper-array elements — both element kinds.**
  ...
  new machinery — to a `string`/`blob` element and to a whole
  `struct`/`union`/nested-array element alike. The exception in the table follows
  from where the length comes from: a wrapper carries no length field, so the
  decoded length is *highest present id + 1* (§5.1) — **nothing that carries the
  length may be elided, and everything else may be.** §5.1 states that rule
  normatively; its consequences:

  `["a", ""]` and `["a"]` are different values with different encodings, and an
  all-default `["", ""]` is written as its final element alone, at id `1` — not as
  the empty array. `[x, default, default]` is written as element `0` plus an
...
- **Empty ≠ absent — at the *array* level.** The wire can now carry an explicit
  empty array and an empty sequence (CORELIB_PLAN §4.7, §4.9), so for an array
  field: *absent* → reconstructed as the declared default (which may be non-empty,
  e.g. `default: [3, 4]`); *explicit empty* → the empty collection, overriding that
  default. This holds for **every** array: a declared `count` bounds how many
  ...
  *interior* element is **indistinguishable from an absent one** (both reconstruct
  to the element default), so omitting element 1 of `["a", "", "c"]` is lossless —
...
buffer, and so an over-long array is invalid. `count` is **optional**, exactly like
`maxlen` — omitting it declares a dynamic/unbounded array (§7.2); the two differ
only in whether that bound exists.

**The wire count `M` is the array's length (normative).** `M` **MUST** satisfy
`0 ≤ M ≤ N`; `M > N` is the `INVALID` decode outcome (§7). A decoder materializes
**exactly the `M` elements the wire carries** — the same value on a pre-sized target
and on a growable one; a target pre-sized to `N` leaves the slots at `[M, N)` at
their element default and reports a length of `M`. There is **no fill-to-`N`**:
`M = 3` in a `count: 5` array is a three-element array, and a `count` is not a
promise that five elements exist. `M = 0` is a valid array — the **explicit empty
array**, the explicit-empty form of §2; its exact wire form (including why an empty
fixlen array still carries its `fixlen_word`) is byte-level encoding and lives in
CORELIB_PLAN §4.7–4.8.

**No interior sparsity, and no trailing elision.** A compact scalar array is
serialized **linearly and gap-free**: the `M` elements named by the count prefix are
all present, in order, with no per-element header. A default-valued element stays on
the wire, trailing ones included — `M` is the length, so eliding one would shorten
the array: `[1, 2, 3, 0, 0]` and `[1, 2, 3]` are different values and encode
differently (`M = 5` and `M = 3`). The id-gap omission of §2/§5.1 applies **only**
to wrapper-sequence arrays (string/blob/struct/union elements), never to these
count-prefixed forms. This is the same principle the wrapper form obeys from the
other side (§5.1): nothing that carries the length may be elided.
...
only to the generated code. An **empty sequence** — one carrying no child — is
legal; what it denotes depends on the position it occupies (§2).
...
only the wrapper can represent an **explicit empty** array — the empty wrapper
sequence, which a field whose declared `default` is non-empty needs in order to
override it (§2, §3).
...
contain gaps** (below); only the **last** index is never a gap. (Languages with
1-based arrays, e.g. Lua, apply the +1 offset in their binding; the wire is always
0-based.)
...
The wrapper sequence carries the array field's own `id` in its parent scope;
whether the field appears at all is the ≠-default test of §2.
...
- an **interior** element equal to its element default **MUST NOT** be written: a
  ...
  framed. The id sequence then has a **gap** — ids such as `0, 2, 3` are
  well-formed, not an error — and a decoder **MUST** accept it. (A decoder still
  accepts a present, default-valued interior element — a `""` leaf or an empty frame
  — for robustness; a conformant encoder never emits one, so the encoding stays
  canonical and a re-encode normalizes it away.)
- the element at the **highest index MUST be written**, whatever its value and
  whatever its kind — a `string`/`blob` as its (default) value, a sequence as an
  **empty frame** — because it is the only one whose presence the length depends on.
  A decoder correspondingly **MUST NOT** treat a present all-default element at that
  index as absent: that presence is what fixes the length, which is precisely why an
  all-default **element** there keeps its frame while an all-default **field** never
  does (§2). This holds for **every** wrapper array, with or without a schema
  `count`: `count` is a capacity, never a length (§3), so it can never restore an
  elided tail.

Before applying a wrapper array a decoder **MUST** initialise every destination slot
to its element default — a target pre-sized to the schema `count`/`maxlen` on
heap-less profiles, or a fresh buffer sized to the transmission — then write each
present
element at `dest[id]`, leaving absent ids at their default. Where the destination
buffer is supplied by the application, handing it over cleanly initialised is the
application's part of that contract.
...
that element is never elided; dropping it would shorten the array, and
`[s, default]` and `[s]` are different values on every target, pre-sized or
growable. A schema `count: N` does **not** change this: it is a **capacity** (§3),
so it bounds the array (an element id `≥ N` is `INVALID`, §7) and lets a heap-less
target pre-size, but it never adds elements a decoder did not receive. A target
pre-sized to `N` therefore reports a length of *highest present id + 1* and leaves
the remaining slots at their element default, recovering the same value a growable
target does. An **interior** default element is indistinguishable from an absent one
...
`""` — that is what fixes the recovered length at 3 (§5.1, last-element rule).
Writing element 1 densely instead would cost its header plus an empty `fixlen_word`
(two bytes) and denote the same value — the non-canonical form a decoder accepts and
a re-encode normalizes away (§2).
...
(a wrapper sequence of two-field structs), so §5.1 applies unchanged: entries are
delimited by the sequence end rather than a length field, each at its index id, the
interior is sparse, the last entry is always written, and the entry count is
*highest present id + 1*. Schema `count` is a **capacity** (§3): a heap target omits
it for an unbounded map; a heap-less target declares it to pre-size, which bounds
the map at `N` entries without asserting that `N` exist — the unused slots of a
pre-sized target stay at their all-default value (the natural "empty slot" sentinel
for a fixed-capacity map).
...
Two consequences of `count` being a capacity (§3):

- `items.count` is **optional** (like `maxlen`): present → the array is **bounded
  at `N`**, and heap-less targets pre-size from it; omitted → dynamic/unbounded.
  Either way it never appears on the wire.
...
- **Enforce schema bounds as `INVALID`.** The corelib cannot know the schema, so
  ...
  width** — including an `enum` or a `bitfield`, each bounded by the width its
  declaration implies (§1). Each is malformed input and **MUST** be reported as
  ...
  masked: neither a scalar nor a bitfield to its declared width.
...
width** of an integer scalar (`u8`…`u64`, `i8`…`i64` — §1), and the **declared
width** an `enum` or a `bitfield` implies (§1) are **wire-validity
